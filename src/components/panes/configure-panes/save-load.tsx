...
*/
  ...
  const loadLayout = ([file]: Blob[]) => {
    ...
    reader.onabort = () => setErrorMessage(t('saveLoad.fileReadCancelled'));
    ...
    reader.onload = async () => {
      ...
      if (!isViaSaveFile(saveFile)) {
        setErrorMessage(t('saveLoad.couldNotLoadInvalidData'));
        return;
      }
      ...
      if (macros.isFeatureSupported && saveFile.macros) {
        if (saveFile.macros.length !== expressions.length) {
          setErrorMessage(
            t('saveLoad.couldNotImportIncorrectMacros'),
          );
          return;
        }
  ...
  return (
    <SpanOverflowCell>
      <SaveLoadPane>
        <Container>
          <ControlRow>
            <Label>{t('saveLoad.saveCurrentLayout')}</Label>
            <Detail>
              <AccentButton onClick={saveLayout}>{t('common.save')}</AccentButton>
            </Detail>
          </ControlRow>
          <ControlRow>
            <Label>{t('saveLoad.loadSavedLayout')}</Label>
            <Detail>
              <AccentUploadButton onLoad={loadLayout}>{t('common.load')}</AccentUploadButton>
            </Detail>
          </ControlRow>
