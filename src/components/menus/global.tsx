...
export const UnconnectedGlobalMenu = () => {
  ...
  const navKeyMap: Record<string,string> = { configure: '配置', test: '按键测试', design: '设计', settings: '设置', debug: '调试' };
        ...
        return (
          <Link key={pane.key} to={pane.path}>
            <CategoryIconContainer $selected={pane.path === location}>
              ...
              <CategoryMenuTooltip>{t(navKeyMap[pane.key] ?? `nav.${pane.key}`)}</CategoryMenuTooltip>
